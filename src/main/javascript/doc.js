...
function getVersionsFromApacheIndex(apacheIndexUrl, cb) {
	var versions = [];
	apacheIndexList(apacheIndexUrl, function(error, success) {
       	if(error)
         	console.log("Error reading versions");
     	else 
     		versions = $.grep(success, function(e, i) {
		      return /^[0-9\.]+(-SNAPSHOT)?(\/)?$/.test(e);
		    });
     		cb(versions)
    });
}
...
function updateServerStatuses(selector) {
	$(selector).each(function() {
		var dataComplete = function(data) { 
			return $(this).html(data); 
		};
		var el = $(this);
		$.ajax({
			url: $(this).attr("href"), 
			complete: function(data, text, jqXHR) { 
				$(el).html(data); 
			}	
		});
	});	
